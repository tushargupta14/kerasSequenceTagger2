...
def create_embedding_matrix(corpus_word_vocab) :
	...
	for k,v in corpus_word_vocab.iteritems() :
		...
		embed_mat[v,:] = np.asarray(corpus_vector_dict[str(v)]) 
	return embed_mat
...
def train_model():
	...
	for i in range(epochs) :
		...
		with open("weights_train_false/model2_epoch_"+str(i)+".json","wb+") as json_file :
			json_file.write(json_object)
		model.save_weights('weights_train_false/model2_weights_epoch_'+str(i)+'.h5')


		with open("results_train_false/avgLoss_list.pkl","wb+") as f:
			cp.dump(avgLoss_list,f)

		with open("results_train_false/score_list.pkl","wb+") as f:
			cp.dump(score_list,f)
		with open("results_train_false/vLoss_list.pkl","wb+") as f:
